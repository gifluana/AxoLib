...
public class Ternary implements IExpression {
    ...
    public IExpression get() {
        if (this.isNumber()) this.result.set(this.doubleValue());
        else                 this.result.set(this.stringValue());
        return this.result;
    }

    @Override public boolean isNumber()     { return this.ifFalse.isNumber() || this.ifTrue.isNumber(); }
